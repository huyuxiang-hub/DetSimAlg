...
${CMTROOT}/mgr/cmt setup -sh -pack=DetSimAlg -version=v0 -path=/junofs/users/huyuxiang/juno_centos7/offline/Simulation/DetSimV2  -no_cleanup $* >${cmtDetSimAlgtempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/mgr/cmt setup -sh -pack=DetSimAlg -version=v0 -path=/junofs/users/huyuxiang/juno_centos7/offline/Simulation/DetSimV2  -no_cleanup $* >${cmtDetSimAlgtempfile}"
  cmtsetupstatus=2
